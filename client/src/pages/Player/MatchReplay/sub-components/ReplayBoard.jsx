...
export default function ReplayBoard({ boardState, boardSize, markerStyle, boardStyle = 'jungle' }) {
    const theme = getTheme(boardStyle);
    
    const columns = useMemo(() => COL_LETTERS.slice(0, boardSize), [boardSize]);
    const rows = useMemo(() => Array.from({ length: boardSize }, (_, i) => String(i + 1).padStart(2, '0')), [boardSize]);
